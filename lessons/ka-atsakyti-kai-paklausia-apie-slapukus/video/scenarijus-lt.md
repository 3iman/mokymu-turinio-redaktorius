# Video scenarijus: Ką atsakyti, kai paklausia apie slapukus

**Formatas:** Lietuviškai, su balsu — iliustracijos + tekstas ekrane + balsas + foninė muzika
**Balsas:** ElevenLabs `eleven_v3`, balsas `eqJHjeWMPGJFD6VBf1J2`, greitis 0.9 — `node generate-voice.js --lesson ka-atsakyti-kai-paklausia-apie-slapukus --lang lt`
**Balso seed:** 1779642367 — pasirinktas Eimanto ausimi; keičiant tekstą kandidatai renkami iš naujo
**Balso tekstas:** atsirinktas iš pamokos teksto (WP), ne rašytas iš naujo. Kirtis „ràštinę“ patvirtintas Eimanto 2026-09-15.
**Trukmė:** ~1:15 min.
**Foninė muzika:** `jiglr-malibu_cleverphant-mokymu-fonas.mp3` (~20% garsumo)

---

| Kadras | Trukmė | Failas | Balsas |
|---|---|---|---|
| 0 Intro | 5 sek. | intro | — |
| 1 Klausimas ir atsakymas | 8 sek. | 00-virselis | Kartą per metus ar dažniau į ràštinę ateina laiškas iš asmens duomenų apsaugos pareigūno. |
| 2 Ko klausiama | 9 sek. | 01-is-kur-ateina-klausimas | Klausiama ne apie tai, ar naudojate slapukus, o apie tai, ar lankytojas turi tikrą pasirinkimą jų atsisakyti. |
| 3 Visur toks pats modulis | 8 sek. | 02-tas-pats-modulis | Reikalavimus stebime mes. Daryti jums nereikia nieko. |
| 4 Kur rasti nuorodą | 10 sek. | 03-kur-rasti-nuoroda | Sutikimą galima bet kada pakeisti per nuorodą Slapukų valdymas svetainės apačioje. |
| 5 Pirmą kartą nepažymėta nieko | 11 sek. | 04-pirma-karta-nepazymeta | Jei lankytojui reikia langelį atžymėti, kad atsisakytų, tai jau ne sutikimas, o numatytoji tvarka. |
| 6 Mygtukai vienodi | 9 sek. | 05-mygtukai-vienodi | Daugelyje svetainių Sutinku yra ryškus spalvotas mygtukas, o Nesutinku – pilkas tekstas šalia. |
| 7 Ką renka | 12 sek. | 06-ka-renka | Google slapukas atsiranda tik tuo atveju, jei mokykla pati naudoja „Google Analytics“. Jei nenaudojate, jo svetainėje nėra. |
| 8 Naują versiją gauna visos | 8 sek. | 07-vienas-taisymas | Naują versiją gauna visos mokyklos – ne tik ta, kuri paklausė. |
| 9 Outro | 5 sek. | autro | — |

---

## Kadrai

### KADRAS 0: Intro (5 sek.)
**Failas:** intro šablonas (generuojamas)
**Ekrane:**
- Cleverphant logo (centras)
- Pavadinimas: Ką atsakyti, kai paklausia apie slapukus

---

### KADRAS 1: Klausimas ir atsakymas (8 sek.)
**Failas:** `00-virselis.png`
**Overlay tekstas:** Techniškai skambantis klausimas. Atsakymas telpa į tris teiginius.

**Komentaras:** Pirmas kadras nuramina — žiūrovas iš karto mato, kad atsakymas jau paruoštas.

---

### KADRAS 2: Ko klausiama (8 sek.)
**Failas:** `01-is-kur-ateina-klausimas.png`
**Overlay tekstas:** Tai ne patikra. Tai klausimas, į kurį pakanka atsakyti raštu.

**Komentaras:** Trijų žingsnių grandinė. Svarbiausia žinutė — nerimauti nereikia.

---

### KADRAS 3: Visur toks pats modulis (8 sek.)
**Failas:** `02-tas-pats-modulis.png`
**Overlay tekstas:** Visoms svetainėms atnaujiname tą pačią modulio versiją

**Komentaras:** Paaiškina, kodėl mokyklai nieko daryti nereikia.

---

### KADRAS 4: Kur rasti nuorodą (10 sek.)
**Failas:** `03-kur-rasti-nuoroda.png`
**Overlay tekstas:** Nuoroda yra kiekvieno puslapio pačioje apačioje

**Komentaras:** Tikras poraštės fragmentas su pažymėta nuoroda. Ilgesnis kadras — žiūrovas ieško jos akimis.

---

### KADRAS 5: Pirmą kartą nepažymėta nieko (10 sek.)
**Failas:** `04-pirma-karta-nepazymeta.png`
**Overlay tekstas:** Pasirinkti turi pats lankytojas — už jį niekas nepažymėta

**Komentaras:** Tikras nustatymų puslapio vaizdas greta antipaternos. Ilgesnis kadras — du dalykai lyginami.

---

### KADRAS 6: Mygtukai vienodi (8 sek.)
**Failas:** `05-mygtukai-vienodi.png`
**Overlay tekstas:** „Nesutinku“ ir „Sutinku“ — to paties dydžio, formos ir spalvos

**Komentaras:** Tikra slapukų juosta. Žiūrovas atpažįsta savo svetainę.

---

### KADRAS 7: Ką renka (10 sek.)
**Failas:** `06-ka-renka.png`
**Overlay tekstas:** Lankytojo vardo ar pašto svetainė nerenka

**Komentaras:** Lentelė su trimis eilutėmis. Ilgesnis kadras — reikia laiko perskaityti.

---

### KADRAS 8: Naują versiją gauna visos (8 sek.)
**Failas:** `07-vienas-taisymas.png`
**Overlay tekstas:** Naują versiją gauna visos mokyklos iš karto

**Komentaras:** Rami pabaiga — mokykla nelieka viena.

---

### KADRAS 9: Outro (5 sek.)
**Failas:** `autro.mp4`
**Komentaras:** Esamas outro failas.

---
